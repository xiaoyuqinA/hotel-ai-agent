...
class TestAgentCreation:
    def test_create_agent_returns_agent(self):
        ...
        from capabilities.guest_experience.agents.review_reply_agent.agent import (
            create_agent,
        )
    ...
    def test_agent_has_output_type(self):
        """Agent 应配置输出类型（纯文本输出，不使用 output_type）。"""
        from capabilities.guest_experience.agents.review_reply_agent.agent import (
            create_agent,
        )

        agent = create_agent()
        # 纯文本输出模式，output_type 为 None
...
class TestReplyResultBusiness:
    def test_positive_reply(self):
        """正向回复场景。"""
        from capabilities.guest_experience.agents.review_reply_agent.schemas import (
            ReplyResult,
        )
